...
public class Chose implements Serializable {
    ...
    private static final long serialVersionUID = 6344462698424468523L;

      //调用add方法，先创建一个学生对象，自动调用构造方法，对学生信息进行赋值，然后将创建的对象添加到集合中
    public  static List<Student> add(List<Student> ls){
        System.out.println("请输入要创建学生的信息");
        Student s = new Student();
        for (Student l : ls) {
            if(l.toString() == s.toString()){
                System.out.println("系统已经存在该学生");
                return ls;
            }
        }
        ls.add(s);
